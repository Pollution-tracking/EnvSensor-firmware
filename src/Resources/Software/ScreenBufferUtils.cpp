...
static String formatSensorValue(int32_t value, const String &unit,
                                bool includeUnit, bool integerOnly) {
  switch (value) {
  case READ_ERROR:
...
String convertHumidity(BMEData data, bool includeUnit, bool integerOnly) {
  ...
  return formatSensorValue(lastSensorsData.bmeCompensatedRH, " %", includeUnit,
                           integerOnly);
#else
...
#ifdef COMPENSATION_ENABLE
  return formatSensorValue(lastSensorsData.coPPMComp, " ppm", includeUnit,
                           integerOnly);
#else
...
#ifdef COMPENSATION_ENABLE
  return formatSensorValue(lastSensorsData.no2PPMComp, " ppm", includeUnit,
                           integerOnly);
#else
...
#ifdef COMPENSATION_ENABLE
  return formatSensorValue(lastSensorsData.nh3PPMComp, " ppm", includeUnit,
                           integerOnly);
#else
