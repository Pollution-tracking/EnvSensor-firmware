...
To train, evaluate, and verify calibration models, set up the Python virtual environment in the project root:
...
2.  **Install Dependencies**:
    ...
    ```
3.  **Mac users (XGBoost support)**:
    XGBoost requires the OpenMP multi-threading library to load its dynamic binaries on macOS. Install it via Homebrew:
    ```bash
    brew install libomp
    ```

### Model Training Workflow
Once activated, you can train and serialize models using reference station data (e.g. Bucharest WAQI open feeds):
```bash
python3 tools/train_calib.py --trace-csv <path-to-logs> --ref-csv <path-to-ref> --target pm25
```
This script evaluates the model and outputs a `.bin` payload to copy directly onto the SD card under `/models/`.
